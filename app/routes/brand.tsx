...
const statusColors: Record<CollabStatus, { bg: string; text: string }> = {
  potential: { bg: "rgba(255,255,255,0.06)", text: "rgba(245,245,245,0.4)" },
...
export default function BrandPage() {
  ...
  if (!brand) return <div className="text-[rgba(245,245,245,0.3)]">Loading brand data...</div>;
          ...
          <div className="card-static p-5">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-[14px] font-medium text-[#f5f5f5]">@flywithgarrett</p>
              </div>
              <div>
                <p className="text-micro mb-1">Tagline</p>
                <p className="text-[14px] font-medium text-[#f5f5f5] italic">"Living at altitude."</p>
              </div>
              <div>
                <p className="text-micro mb-1">Tone</p>
                <div className="flex flex-wrap gap-1.5">
                  {["Authentic", "Aspirational", "Approachable"].map((t) => (
                    <span key={t} className="pill px-2.5 py-1 text-[12px] text-[rgba(245,245,245,0.55)]">{t}</span>
                  ))}
                </div>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {["The Flight Deck", "The Life", "The Build"].map((w) => (
                    <span key={w} className="pill px-2.5 py-1 text-[12px] text-[rgba(245,245,245,0.55)]">{w}</span>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
              {Object.entries(PILLAR_CONFIG).map(([key, val]) => (
                <div key={key} className="text-center">
                  ...
                  <p className="text-xs font-medium text-[#f5f5f5]">{val.label}</p>
                  <p className="text-[10px] text-[rgba(245,245,245,0.3)]">{val.color}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            return (
              <div key={platform} className="card-static p-5">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-[13px] font-medium text-[#f5f5f5]">{PLATFORM_CONFIG[platform].label}</span>
                </div>
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={product.name} className="flex items-center gap-3 p-3 rounded-lg bg-[rgba(255,255,255,0.04)]">
                  ...
                  <span className="text-[14px] font-medium text-[#f5f5f5]">{product.name}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div>
          ...
          {collabs.length === 0 ? (
            <div className="card-static p-12 text-center">
              <Handshake className="w-8 h-8 text-[rgba(245,245,245,0.3)] mx-auto mb-2" />
                ...
                return (
                  <div key={collab.id} className="card-static p-4">
                    <div className="flex items-center justify-between mb-2">
                      <p className="font-medium text-sm text-[#f5f5f5]">{collab.creatorName}</p>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center">
              ...
              <div className="relative w-full max-w-md card-static p-6">
                <div className="flex items-center justify-between mb-5">
                  <h2 className="text-[16px] font-medium text-[#f5f5f5]">New Collaboration</h2>
                  <button onClick={() => setShowAddCollab(false)} className="p-1 hover:bg-[rgba(255,255,255,0.06)] rounded">
