        ...
        <div className="flex rounded-[10px] bg-[rgba(255,255,255,0.04)] p-1">
          <button
            ...
            className={cn(
              ...
              view === "month" ? "bg-[rgba(255,255,255,0.06)] text-[#f5f5f5]" : "text-[rgba(245,245,245,0.3)] hover:text-[#f5f5f5]"
            )}
          >
            Month
          </button>
          <button
            ...
            className={cn(
              "px-4 py-1.5 text-[13px] font-medium rounded-lg transition-colors",
              view === "week" ? "bg-[rgba(255,255,255,0.06)] text-[#f5f5f5]" : "text-[rgba(245,245,245,0.3)] hover:text-[#f5f5f5]"
            )}
          >
            Week
          </button>
      ...
      {view === "month" && (
        <div className="grid grid-cols-7 gap-px rounded-2xl overflow-hidden" style={{ backgroundColor: "transparent" }}>
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
            <div key={d} className="bg-[#1e1e1e] p-2 text-center text-micro">
              {d}
            </div>
          ))}
          {Array.from({ length: firstDay }).map((_, i) => (
            <div key={`empty-${i}`} className="bg-[#141414] min-h-[100px] p-1" />
          ))}
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isToday && "ring-1 ring-white/10 ring-inset"
                )}
              >
                <div className="flex items-center justify-between mb-1">
                  <span className={cn("text-[11px] font-medium", isToday ? "text-[#f5f5f5]" : "text-[rgba(245,245,245,0.3)]")}>{day}</span>
                  {cadence && (
                    <span
                ...
                <div className="space-y-0.5">
                  ...
                  {dayItems.length > 3 && (
                    <span className="text-[10px] text-[rgba(245,245,245,0.3)] px-1">+{dayItems.length - 3} more</span>
                  )}
                </div>
              </div>
            );
            ...
            return (
              ...
              >
                <div className="p-3 pb-1">
                  ...
                  <div className={cn("text-sm font-semibold", isToday ? "text-[#f5f5f5]" : "text-[rgba(245,245,245,0.55)]")}>
                    {day.getDate()}
                  </div>
                  {cadence && (
                    <span
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1 mt-1">
                          {item.platforms.map((p) => {
                            ...
                            return Icon ? <Icon key={p} className="w-3 h-3 text-[rgba(245,245,245,0.3)]" /> : null;
                          })}
                        </div>
                      </button>
                    ))
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-md card-static p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-[16px] font-medium text-[#f5f5f5]">New Content Item</h2>
              <button onClick={() => setShowCreate(false)} className="p-1 hover:bg-[rgba(255,255,255,0.06)] rounded">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  {PLATFORMS.map((p) => {
                    const Icon = platformIcons[p];
                    return (
                      <label key={p} className="flex items-center gap-1.5 text-[13px] text-[rgba(245,245,245,0.55)]">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-md card-static p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-[16px] font-medium text-[#f5f5f5]">{selectedItem.title}</h2>
              <button onClick={() => setSelectedItem(null)} className="p-1 hover:bg-[rgba(255,255,255,0.06)] rounded">
                <X className="w-4 h-4 text-[rgba(245,245,245,0.3)]" />
              </button>
            </div>
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                {selectedItem.platforms.map((p) => {
                  const Icon = platformIcons[p];
                  return (
                    <span key={p} className="flex items-center gap-1 text-xs text-[rgba(245,245,245,0.55)]">
